...
<tr><th>ID</th><th>フラグ</th><th>作業実績日</th><th>手配番号</th><th>機械コード</th><th>日勤夜勤区分</th><th>備考</th>
<th></th></tr>

<%for(Product p:list) {%>
<tr><th><%= p.getId() %></th><td><%= p.getFlag()%></td><td><%= p.getWorkperformancedate() %></td><td><%= p.getArrangementnumber() %></td>
...
<% if(p.getFlag() == 1 || p.getFlag() == 2) { %>
     <a id="downloadBtn_<%= p.getId() %>" href="<%= request.getContextPath() %>/Ingestiondata?action=<%= p.getFlag() == 1 ? "download" : "download2" %>&id=<%= p.getId() %>" class="btn btn-success" onclick="return confirmDownload('ダウンロードしてよろしいですか？', <%= p.getId() %>);">CSV</a>
<% } %>
